...
- **Content plan:** MOSS introduction → battery reaches zero → MarsCoin restores him → his new search → planned 3% mechanics → MarsCoin Expedition preview → launch coordinates.
...
- [`marscoin-expedition-gameplay.png`](public/assets/marscoin-expedition-gameplay.png) is the supplied gameplay concept image. The Expedition layout, copy, and feature panels are HTML/CSS; the second supplied image is only a visual reference.
- The planned 3% total fee is presented as 2% holder rewards, 0.5% Expedition fund, and 0.5% development. The volume examples are illustrative total holder pools, not individual payouts.
...
2. Confirm the planned 3% fee allocation and how Flap's protocol fee affects it before claiming an exact realized percentage.
